...
def _tool_call_markup(name: str, inputs: dict, dry_run: bool, agent_label: str | None, mode_badge: str | None) -> str:
    ...
    for k, v in inputs.items():
        ...
        if isinstance(v, str) and "\n" in v:
            n = v.count("\n") + 1
            block_lines.append(f"[#444444]│[/]   [muted]{k} ({n} lines):[/]")
            for line in v.splitlines():
                block_lines.append(f"[#444444]│[/]   [muted]│[/] {line}")
        elif isinstance(v, str) and len(v) > 60:
            inline_args.append(f"[muted]{k}=[/][{BLUE}]\"{v[:50]}…\"[/]")
        else:
            inline_args.append(f"[muted]{k}=[/][{BLUE}]{v!r}[/]")

    header = f"[#444444]│[/] {agent_prefix}[#888888]⚙[/]  {label}[{name_color}]{name}[/]{badge_str}  {'  '.join(inline_args)}"
    if block_lines:
        return header + "\n" + "\n".join(block_lines)
    return header
...
def _tool_result_markup(result: str) -> str:
    ...
    if result.startswith("Error:"):
        return f"[#444444]│[/]   [bold red]└─ Error:[/] {escape(result[7:].strip())}"
