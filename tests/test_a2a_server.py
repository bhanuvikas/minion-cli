...
def _wait_for_status(port: int, task_id: str, target: str, timeout: float = 5.0) -> dict:
    """Poll GET /tasks/{id} until status reaches target or timeout."""
    deadline = time.monotonic() + timeout
    while True:
        status, data = _get(port, f"/tasks/{task_id}")
        if status == 200 and data.get("status") == target:
            return data
...
class TestA2AServer(unittest.TestCase):
    ...
    def test_get_task_returns_submitted_when_pending(self):
        # Submit a slow task so we can observe submitted state
        # (In practice the task may complete immediately with lambda)
        _, submit_data = _post(self.port, "/tasks/send", {"message": "fast task"})
        task_id = submit_data["id"]
        # Even if it completes quickly, task should exist
        status, data = _get(self.port, f"/tasks/{task_id}")
        self.assertEqual(status, 200)
        self.assertIn(data["status"], ["submitted", "working", "completed"])
    ...
    def test_get_task_returns_completed_with_artifact(self):
        ...
        artifact_text = completed["artifacts"][0]["text"]
    ...
    def test_send_subscribe_streams_sse_events(self):
        ...
        # Completed event should have an artifact
        final = next(e for e in events if e.get("status") == "completed")
    ...
    def test_concurrent_tasks_handled_independently(self):
        ...
        self.assertIn("task-alpha", results["task-alpha"]["artifacts"][0]["text"])
        self.assertIn("task-beta", results["task-beta"]["artifacts"][0]["text"])
...
class TestEchoAgent(unittest.TestCase):
    ...
    def test_echo_agent_send_task_completes(self):
        ...
        self.assertIn("ping", done["artifacts"][0]["text"])
